    ...
    override fun onBindViewHolder(holder: GameLevelViewHolder, position: Int) {
      ...
      //  holder.txtTimes.text = gameLavelList.get(position).times.toString()
       ...
       holder.btnTryIt.setOnClickListener{
            if (gameLevelList[position].title == "Equation Level"){
                var intent =  Intent(contex, SolveEquation::class.java)
                contex.startActivity(intent);
            }
           if (gameLevelList[position].title == "Memory Level"){
               var intent =  Intent(contex, MemoryGame::class.java)
               intent.putExtra("level", 0)
               contex.startActivity(intent);
           }


           if (gameLevelList[position].title == "Shape Sequence  Level"){
             //  var intent =  Intent(contex, SolveEquation::class.java)
               //contex.startActivity(intent);
           }

           if (gameLevelList[position].title == "Type Level"){
               var intent =  Intent(contex, Type::class.java)
               contex.startActivity(intent);
           }
        }
